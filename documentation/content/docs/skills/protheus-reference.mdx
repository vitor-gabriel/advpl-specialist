...
5. **Validacao de campos:** Para validar se um campo `ALIAS_CAMPO` existe em uma tabela padrao, usar CQL do tdn-lookup: `type=page AND title~"{TABLE_ALIAS}" AND text~"{FIELD_NAME}"`. Se o campo nao for confirmado, perguntar ao usuario ou usar variavel `cx*` com TODO.
...
Esta referencia inclui os seguintes arquivos de referencia detalhados:
