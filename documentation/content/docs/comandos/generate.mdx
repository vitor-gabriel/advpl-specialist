...
6. **Resolver metadados do ponto de entrada** - **Se o tipo for `ponto-entrada`**, resolve PARAMIXB, tipo de retorno, rotina chamadora e momento de execucao nesta ordem:
   1. **Tier 1 — cache local primeiro:** le `skills/advpl-code-generation/catalogo-top-50-pes.md` e procura o nome exato do PE. Se encontrado com metadados completos, extrai PARAMIXB, retorno, rotina chamadora e momento direto do catalogo. **Nao consulta o TDN.** Cobertura inicial: 20 PEs nos modulos COM, FAT, EST e FIN.
   2. **Tier 2+ — fallback TDN:** se o PE nao esta no catalogo local, aplica a estrategia completa documentada em `skills/tdn-lookup/reference.md` (WebFetch na API REST do Confluence, depois Playwright como fallback). Extrai os mesmos metadados da pagina do TDN.

   Essa estrategia cache-first reduz a latencia em ~70% para os PEs mais comuns e mantem o TDN como fallback para garantir correcao. O codigo gerado deve citar a fonte usada (catalogo ou URL do TDN) em comentario, para que revisores consigam rastrear de onde veio o PARAMIXB.
...
8. **Apresentar plano** - Mostrar ao usuario um plano claro incluindo:
   ...
   - **Para pontos de entrada:** fonte dos metadados (catalogo local `catalogo-top-50-pes.md` ou URL especifica do TDN) — parametros PARAMIXB, tipo de retorno, rotina chamadora e momento de execucao
...
# Para PEs comuns (catalogados), o plugin responde em ~70% menos tempo
# que no v1.1.1 porque consulta cache local antes do TDN
...
## Cache local de pontos de entrada

Disponivel a partir da v1.1.2. Quando voce gera um ponto de entrada com `/advpl-specialist:generate ponto-entrada NOME`, o plugin aplica uma estrategia de lookup em 2 niveis:

1. **Tier 1 — cache local:** consulta o arquivo `skills/advpl-code-generation/catalogo-top-50-pes.md` do plugin. Se o PE estiver catalogado (cobertura inicial: 20 PEs em COM, FAT, EST, FIN), extrai PARAMIXB, momento, rotina chamadora e retorno diretamente. **Nao consulta o TDN.**
2. **Tier 2+ — fallback TDN:** se o PE nao esta no cache local, cai na estrategia completa da skill `tdn-lookup` (API REST do Confluence + fallbacks Playwright)

**Ganho medido:** ~70% menos latencia para PEs comuns, porque o cache responde em zero tokens de rede.

### PEs atualmente catalogados

- **COM:** MT100TOK, MT100GRV, MT103FIM, MA103OK, MT097APR
- **FAT:** M410CMPL, M410STTS, MA440GRV, M460FIM, MT460A
- **EST:** MA030TOK, MT240PRO, MT116NT, MT250GRV, MA030GRV
- **FIN:** F050TIT, F080FIL, M460MVF, MF370FIL, F240FIL

Outros PEs continuam funcionando via TDN — so nao tem o boost do cache.

### Como sugerir novos PEs para o catalogo

Quer que um PE especifico entre no catalogo? Abra uma issue em [github.com/thalysjuvenal/advpl-specialist/issues](https://github.com/thalysjuvenal/advpl-specialist/issues) informando:

- **Nome do PE** e **rotina chamadora** (ex.: `MA440CAN` na `MATA440`)
- **PARAMIXB** real (validado contra TDN — o plugin **nao aceita PEs inventados no catalogo**)
- **Link TDN** da documentacao oficial
- **Modulo** e **momento** de execucao

Roadmap para chegar a 50 PEs catalogados em `skills/advpl-code-generation/catalogo-top-50-pes.md`.
